...
const renderDetails = () => (
  ...
  >
    {Staking.details.map((detail, idx) => (
      <Box key={idx} sx={{ display: 'flex', marginBottom: '30px' }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', width: '700px'}}>
      ...
      >
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column' }}>
          <Label sx={{ marginLeft: '40px', minWidth: '75px' }} text={Staking.label} />
          {Staking.value && <Label sx={{ marginLeft: '40px' }} text={Staking.value} />}
        </Box>
      </Box>
